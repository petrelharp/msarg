...
```{r zero_circles}
ga <- modify_migration(ga,layer.ind=1,doutM=rexp(ncol(ga)*nrow(ga)),dinM=rexp(nrow(ga)*ncol(ga)))
plot(ga)
...
dem[[1]] <- modify_migration(dem[[1]],layer.ind=1,doutM=rexp(prod(dim(dem[[1]])[1:2]),dinM=rexp(prod(dim(dem[[1]])[1:2]))
